...
class DermsClientTestCase(unittest.TestCase):
    ...
    #     return mocker.patch('derms_app.derms_client.Client')
    #
    # @pytest.fixture
    # def mock_etree(mocker):
    #     return mocker.patch('derms_app.derms_client.etree')
    #
    # @pytest.fixture
    # def mock_history_plugin(mocker):
    #     return mocker.patch('derms_app.derms_client.HistoryPlugin')

    @mock.patch('derms_app.derms_client.HistoryPlugin')
    @mock.patch('derms_app.derms_client.etree')
    @mock.patch('derms_app.derms_client.Client', autospec=True)
    def test_get_devices(self, mock_zeep_client, mock_etree, mock_history_plugin):
        ...
        assert value == ["SynchronousMachine", "Solar", "Battery"]

    @mock.patch('derms_app.derms_client.datetime')
    @mock.patch('derms_app.derms_client.uuid')
    @mock.patch('derms_app.derms_client.get_service')
    @mock.patch('derms_app.derms_client.HistoryPlugin')
    @mock.patch('derms_app.derms_client.etree')
    @mock.patch('derms_app.derms_client.Client', autospec=True)
    def test_create_group(self, mock_zeep_client, mock_etree, mock_history_plugin, mock_get_service, mock_uuid, mock_datetime):
        ...
        value = derms_client.create_group(mrid, name, device_mrid_list)
    ...
    def test__build_endpoint_header(self, mock_uuid):
        ...
        value = derms_client._build_endpoint_header(verb)
    ...
    def test_get_service(self, mock_zeep_client):
        ...
        client.create_service.assert_called_with('{der.pnnl.gov}CreateDERGroupsService', 'http://127.0.0.1:8008/createDERGroups')
        assert  value == 'true'
    ...
    def test_create_groups(self, mock_zeep_client, mock_etree, mock_history_plugin, mock_get_service, mock_uuid, mock_datetime):
        ...
        group_list = [group.Group('group_mrid', 'group_name', device_mrid_list)]
        ...
        mock_get_service.assert_called_once_with(mock_zeep_client.return_value, "create")
        self.assertTrue(mock_get_service.return_value.CreateDERGroups.called, "Failed to call CreateDERGroups().")
        mock_get_service.return_value.CreateDERGroups.assert_called_once_with(headers, body)
